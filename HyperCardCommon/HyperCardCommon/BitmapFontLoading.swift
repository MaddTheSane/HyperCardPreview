...
extension BitmapFont: ResourceContent {
    ...
    private static func loadGlyphs(in data: DataRange) -> [Glyph] {
        ...
        /* Gather some data */
        let firstCharacterCode: Int = data.readUInt16(at: 0x2)
        let lastCharacterCode: Int = data.readUInt16(at: 0x4)
        let maximumAscent: Int = data.readUInt16(at: 0x12)
        let maximumKerning: Int = data.readSInt16(at: 0x8)
        let fontRectangleHeight: Int = data.readUInt16(at: 0xE)
        let bitImageRowWidth: Int = data.readUInt16(at: 0x18)
